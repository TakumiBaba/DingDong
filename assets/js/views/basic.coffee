...
class App.View.Sidebar extends Backbone.View
  el: "#sidebar"
  events:
    "click ul.following li": "followingChange"
  ...
  render: (model)->
    console.log model.get('person').following
    _.each model.get('person').following, (f)=>
      options =
        id: f.id
        source: f.profile_image_urls[0]
        name: f.name
      html = window.JST['sidebar/follow_thumbnail'](options)
      $("ul.following").append html

  followingChange: (e)=>
    console.log e
    console.log @
    $(@.el).find('ul.following li').each ()->
      console.log @
      # if $(@).hasClass 'active'
      $(@).removeClass 'active'
